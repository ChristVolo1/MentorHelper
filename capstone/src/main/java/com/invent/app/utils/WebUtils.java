...
public class WebUtils{
	...
	public void sendMail(String to, String msg, String subject) {
	 ...
	 public void addServicePhoto(MultipartFile file, long id, String folder) throws IllegalStateException, IOException {
	     try {
			 // save dir
	    	 String destDir = request.getSession().getServletContext().getRealPath(UPLOADED_FOLDER)+ File.separator +folder+ File.separator +id+ File.separator +"services";
	    	 log.info("This is path {}", destDir);
			 ...
			 if(!dir.exists()){                                    	 
				...
				String img= servicesRepository.findById(id).get().getImage();
			    ...
			    //save to file system and db
			    multipartFile.transferTo(destination);				
			    servicesRepository.findById(id).ifPresent(
			    		a->{
			    		a.setImage(parseMd5);			    		
			    		});
			  } catch (Exception e) {
